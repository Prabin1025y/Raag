                ...
                currentAlbum?.songs.map((song, index) => {
                    ...
                    return (
                        <React.Fragment key={song._id}>
                            <div onClick={() => handlePlaySong(index, song._id)} className={`grid grid-cols-[25px_5fr_1fr] md:grid-cols-[50px_3fr_1fr_1fr] lg:grid-cols-[50px_2fr_2fr_1fr] px-2 lg:px-4 items-center rounded-md hover:bg-[#492668] ${isCurrentPlayingSong ? "text-rose-500" : "text-[#f2e6ff]"} font-[Poppins] mx-4 transition duration-200 cursor-pointer`}>
                                ...
                                <div className='flex items-center font-[Roboto] gap-2 px-1 lg:px-3 py-1 '>
                                    ...
                                    <div className='flex flex-col justify-center'>
                                        <p className='text-nowrap md:text-wrap text-sm'>{song.title}</p>
                                        <p className='text-xs lg:text-sm text-[#BC87FA]'>{song.artist}</p>
                                    </div>
                                </div>
                                <p className="mx-auto text-xs gap-1 items-center hidden md:flex "><Clock size={12} />{formatTime(song.duration)}</p>
