        ...
        <div className="flex flex-col gap-3 h-[calc(100vh-120px)] mt-3 ml-3">
            <div className="rounded-md bg-[#3B1E54] h-36 p-2 md:p-3 flex flex-col justify-between">
                ...
                <Link to="/" className="flex group gap-3 transition relative duration-200 hover:bg-[#462464] rounded-md px-1 md:px-3 py-2 mt-2">
                    <div className="h-10 w-1 group-hover:scale-y-100 scale-y-0 transition duration-200 rounded-md bg-white/80 absolute -left-[2px] md:left-0 top-0 " />
                    <House className='size-5 lg:size-6  hidden md:inline' />
                    <p className="font-bold text-sm lg:text-base font-[Roboto]">Home</p>
                </Link>
                <Link to="/discover" className="flex group relative gap-3 transition duration-200 hover:bg-[#462464] rounded-md px-1 md:px-3 py-2">
                    <div className="h-10 w-1 group-hover:scale-y-100 scale-y-0 transition duration-200 rounded-md bg-white/80 absolute -left-[2px] md:left-0 top-0 " />
                    <Compass className='size-5 min-w-5 lg:size-6  hidden md:inline' />
            ...
            <div className="rounded-md bg-[#3B1E54] flex-1 flex flex-col overflow-hidden">
                <div className="h-16 flex items-center gap-2 lg:gap-3 px-3 md:px-6 py-2">
                    <DiscAlbum className='size-5 lg:size-6 hidden md:inline' />
                ...
                <ScrollArea className='flex-1 bg-[#281538] py-2'>
                    {isLoading
                        ...
                        : (
                            albums.map(album => (
                                <Link key={album._id} to={`/album/${album._id}`} className='flex flex-col md:flex-row items-center font-[Roboto] gap-2 px2 md:px-3 py-1 rounded-md transition duration-200 hover:bg-[#361c4b]'>
                                    <img className='size-12 md:size-8 min-w-8 lg:size-16 lg:min-w-16 rounded-md' src={album.imageUrl} alt="album thumbnail" />
                                    <div className='flex flex-col justify-center'>
                                        <p className='text-sm lg:text-base'>{album.title}</p>
                                        <p className='text-xs lg:text-sm text-[#e4c3ff] hidden md:inline'>{album.artist}</p>
                                    </div>
                                </Link>
                            ))
                        )
